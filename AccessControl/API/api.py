...
def api_reg():
    ...
    if request.method == 'POST':
        ...
        person = classes.Person(identifiacion_document=identification_doc, first_name=first_name,
                                last_name=last_name, birth_date=birth_date, is_employee=is_employee)
        ...
        if is_employee == True:
            ...
            employee = classes.Employee(
                id=person.id, position=position, salary=salary, email=email, start_date=start_date)
            crud.add_entry(employee)

        # Valores pendientes
        pict = classes.Picture(id=person.id, picture_bytes='', face_bytes='')
        crud.add_entry(pict)
